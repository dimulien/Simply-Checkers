...
namespace Checkers
{
    ...
    {/*
      ...
      */
        ...
        private Move GetBestMove(List<Move> lstOfMoves) // Выдает ход с максимальной оценкой профита. По сути обычная сортировка, поиск максимума в списке
        {
        ...
        * с учетом произведенных ходов в текущей комбинации */
        ...
        private void FindUnprotectedMove(List<Move> unprotectLstOfMoves)
        {
            ...
            foreach (Move item in unprotectLstOfMoves)
            {
                ...
                if (IsCheckerUnderAttack(tmpChecker, ref capString))
        ...
        private bool IsNewLocationOfMultiAttack(Move curMove)
        {
            ...
            if (countOfMoves > 0)
            {
                if (curMove.lstOfAttackMoves[countOfMoves - 1].MoveFrom != curMove.MoveTo)
                    flag = true;
            }
            else
            {
